...
def load_model(weights):
    words = L.Input(shape=(MAX_LEN, ), name='input_layer')
    x = L.Embedding(weights.shape[0], weights.shape[1], weights=[weights], trainable=False, name='embedding_layer')(words)
    x = L.Flatten()(x)
    x = L.Dense(512, activation='relu', name='dense1')(x)
    x = L.Dense(256, activation='relu', name='dense2')(x)
...
if __name__ == "__main__":
    ...
    checkpoint = keras.callbacks.ModelCheckpoint(
        'nn.model.h5', save_best_only=True, verbose=1)
